...
//    THRIFT_HOST                    (TType.STRING,  (short)(Short.MIN_VALUE+7));
    ...
    DapengHeader(byte type, short id) {
        this.type = type;
        this.id = (short)id;
    }
     ...
     */
    public static DapengHeader findThriftHeaderKeyById(short id) {
        for (DapengHeader headerKey : DapengHeader.values()) {
            if (headerKey.id == id) {
                return headerKey;
            }
        }
